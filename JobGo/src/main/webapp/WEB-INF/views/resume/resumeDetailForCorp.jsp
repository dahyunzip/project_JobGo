...
<section class="section resume-detail-section py-5">
	<h2 class="pageTitle">지원자 상세 이력서</h2>
	<div class="container">
		<div class="boardHeader">
			...
			<div class="applicant-profile">
				<div class="pfImg">
					<c:choose>
						<c:when test="${not empty resume.storedFileName}">
							<img src="/upload/${resume.storedFileName}" alt="프로필"
				...
				<div class="pfTxt">
					<p>
						<strong>이름 </strong> ${resume.userName}
					</p>
					<p>
						<strong>이메일 </strong> ${resume.userEmail}
					</p>
					<p>
						<strong>연락처 </strong> ${resume.userPhone}
					</p>
				</div>
				<div class="setState" style="margin: 0 auto;">
					<div class="d-flex align-items-center">
						<select id="statusSelect" class="form-select w-auto me-3"
							style="min-width: 180px;">
							<option value="APPLIED"
								${resume.status eq 'APPLIED' ? 'selected' : ''}>지원완료</option>
							<option value="REVIEWING"
								${resume.status eq 'REVIEWING' ? 'selected' : ''}>검토중</option>
							<option value="PASSED"
								${resume.status eq 'PASSED' ? 'selected' : ''}>합격</option>
							<option value="REJECTED"
								${resume.status eq 'REJECTED' ? 'selected' : ''}>불합격</option>
							<option value="HIRED"
								${resume.status eq 'HIRED' ? 'selected' : ''}>채용완료</option>
						</select>
			...
			<%-- <!-- 지원 상태 변경 -->
		<div class="status-section mb-4 p-3 border rounded mt-30 d-flex align-items-center">
			<div>
				<h5 class="mb-3">입사지원 상태 변경</h5>
				<p>
					현재 상태: <span id="currentStatus"> <c:choose>
		...
		<!-- 자기소개 -->
		<h5 class="mt-30 mb-10">자기소개서</h5>
		<c:if test="${not empty resume.selfIntro}">
			<div class="card card-body">
				<p>
					<strong>지원동기</strong><br> ${resume.selfIntro.motivation}
				</p>
				<p>
					<strong>강점 및 역량</strong><br> ${resume.selfIntro.strengths}
				</p>
				<p>
					<strong>주요 경험</strong><br> ${resume.selfIntro.keyExperience}
				</p>
				<p>
					<strong>입사 후 포부</strong><br> ${resume.selfIntro.futurePlan}
				</p>
			</div>
		</c:if>
		...
		<div class="mt-5 text-center button">
			<a href="/application/corp/applicants" class="btn btn-secondary">목록으로</a>
		</div>
